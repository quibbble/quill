...
export const Log = forwardRef((props, ref) => {
    ...
    return (
        <div className="h-full flex flex-col bg-zinc-900 rounded-sm">
            <div className="font-bold mb-1 px-2 pt-2 text-sm md:text-base">Logs</div>
            <div className="bg-zinc-900 p-2 w-full h-full rounded-sm" { ...other }>
                ...
                { 
                    logs ? logs.map((log, i) => 
                        <div className={`bg-zinc-800 text-${ log.Team }-500 p-1 text-2xs md:text-xs rounded-sm`} key={ i }>{ build(log) }</div>
                    ) : null
                }
                    <div ref={logsEndRef} />
                </div>
